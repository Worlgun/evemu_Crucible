...
void DestinyManager::SetPosition(const GPoint &pt, bool update /*false*/) {
    ...
    GPoint newPosition = pt;
    if (pt.isZero()) {
        _log(DESTINY__TRACE, "Destiny::SetPosition() - %s(%u) point is zero", mySE->GetName(), mySE->GetID());
        EvE::traceStack();
        // this *should* be systemID...
        newPosition = sMapData.GetRandPointOnPlanet(mySE->GetLocationID());
    }

    // **NOUVELLE FONCTIONNALITÉ : Contrainte de maintien dans les bulles d'astéroïdes**
    // Empêcher UNIQUEMENT la sortie de bulle si le joueur est dans les 800km du centre
    if (mySE->HasPilot() && mySE->SysBubble() != nullptr && mySE->SysBubble()->IsBelt()) {
        SystemBubble* pBubble = mySE->SysBubble();
        GPoint bubbleCenter = pBubble->GetCenter();
        double currentDistanceFromCenter = bubbleCenter.distance(mySE->GetPosition());
        
        // Limite de 800km (800,000 mètres) depuis le centre de la bulle
        const double MAX_BELT_DISTANCE = 800000.0;
        
        // Si le joueur est actuellement dans les 800km ET essaie de sortir de la bulle
        if (currentDistanceFromCenter <= MAX_BELT_DISTANCE && !pBubble->InBubble(newPosition)) {
            // EMPÊCHER UNIQUEMENT la sortie de bulle, mais permettre le mouvement interne
            // Calculer la position la plus proche à la limite de la bulle
            GVector direction(mySE->GetPosition(), newPosition);
            direction.normalize();
            
            // Trouver le point d'intersection avec la limite de la bulle
            double bubbleRadius = pBubble->GetRadius();
            GPoint bubbleBoundary = bubbleCenter + (direction * (bubbleRadius - 1000.0)); // 1km de marge
            
            // Permettre le mouvement jusqu'à la limite de la bulle
            newPosition = bubbleBoundary;
            
            // Informer le joueur de la contrainte (seulement en mode non-WARP)
            if (m_ballMode != Destiny::Ball::Mode::WARP && mySE->GetPilot()) {
                mySE->GetPilot()->SendNotifyMsg("Vous devez vous éloigner à plus de 800km du centre de la ceinture d'asteroides pour pouvoir la quitter.");
            }
            
            _log(DESTINY__MESSAGE, "DestinyManager::SetPosition() - %s(%u) mouvement limité à la bordure de bulle. Distance du centre: %.2f km (< 800km)", 
                 mySE->GetName(), mySE->GetID(), currentDistanceFromCenter / 1000.0);
        }
    }
    ...
    // Appeler CheckBubble pour vérifier si l'entité doit changer de bulle
    if (mySE->HasPilot()) {
        sBubbleMgr.CheckBubble(mySE);
    }
