...
void BubbleManager::CheckBubble(SystemEntity *pSE) {
    ...
    if (pBubble != nullptr) {
        ...
        // Empêcher la sortie de bulle si le joueur est dans les 800km du centre d'une ceinture d'astéroïdes
        if (pSE->HasPilot() && pBubble->IsBelt()) {
            GPoint bubbleCenter = pBubble->GetCenter();
            double distanceFromCenter = bubbleCenter.distance(pSE->GetPosition());
            const double MAX_BELT_DISTANCE = 800000.0; // 800km en mètres
            
            if (distanceFromCenter <= MAX_BELT_DISTANCE) {
                // Le joueur est dans les 800km du centre de la ceinture - FORCER le maintien dans la bulle
                _log(DESTINY__BUBBLE_DEBUG, "BubbleManager::CheckBubble() - Player '%s'(%u) is within %.0f km of asteroid belt center (%.2f km). FORCING to stay in bubble %u.",\
                     pSE->GetName(), pSE->GetID(), MAX_BELT_DISTANCE/1000.0, distanceFromCenter/1000.0, pBubble->GetID());
                
                // Notifier le joueur de la contrainte
                if (pSE->GetPilot()) {
                    pSE->GetPilot()->SendNotifyMsg("Vous devez vous éloigner à plus de 800km du centre de la ceinture d'asteroides pour pouvoir la quitter.");
                }
                
                // EMPÊCHER la sortie de bulle en retournant immédiatement
                return;
            }
        }
